...
/**
 * Computes the checksum of one block of data of the given length.
 * Assumes chksum_str arguments points to a CHKSUMFILE_STRINGSIZE byte array.
 * Returns -1 if it encounters an error.
 */
int chksumblock(const struct unixfilesystem *fs, char buf[], int len, char *chksum_str);

/**
 * Computes the checksum of a inumber with extra error reporting.
 * Assumes chksum arguments points to a
 * CHKSUMFILE_SIZE byte array.  Returns the length of the checksum, or:
 *
 * -1 if inode_iget fails
 * -2 if file_getblock fails (also puts the block number of the erroring call in filegetblock_error_param)
 * -3 if it encounters another error.
 */
...
/**
 * Compute the checksum of the specified pathname with extra error reporting.
 * Assumes chksum points to a
 * CHKSUMFILE_SIZE byte array. Returns the length of the checksum, or:
 *
 * -1 if inode_iget fails
 * -2 if file_getblock fails (also puts the block number of the erroring call in filegetblock_error_param)
 * -3 if pathname_lookup fails
 * -4 if it encounters another error
 */
